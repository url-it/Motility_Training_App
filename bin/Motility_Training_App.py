...
def write_config_file(name):
    ...
    # print("Motility_Training_App.py: ------- sub.numx, sub.numy = ", sub.numx, sub.numy)
...
def run_sim_func(s):
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 Motility_Training_App-r7 config.xml")   # "-r7" suffix??
        else:
